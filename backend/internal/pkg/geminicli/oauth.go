...
func EffectiveOAuthConfig(cfg OAuthConfig, oauthType string) (OAuthConfig, error) {
	...
	if effective.ClientID == "" && effective.ClientSecret == "" {
		...
		if v, ok := os.LookupEnv(GeminiCLIOAuthClientIDEnv); ok && strings.TrimSpace(v) != "" {
			builtInClientID = strings.TrimSpace(v)
		}
		secret := strings.TrimSpace(GeminiCLIOAuthClientSecret)
		if v, ok := os.LookupEnv(GeminiCLIOAuthClientSecretEnv); ok && strings.TrimSpace(v) != "" {
			secret = strings.TrimSpace(v)
		}
		if builtInClientID == "" || builtInClientID == "builtin-gemini-cli-client-id-via-env" {
		...
		if secret == "" || secret == "builtin-gemini-cli-client-secret-via-env" {
