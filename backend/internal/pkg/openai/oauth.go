...
type OpenAIAuthClaims struct {
	...
	SubscriptionExpiresAtRaw any                 `json:"subscription_expires_at"` // may be string (RFC3339) or number (Unix timestamp)
}
...
// ExtractSubscriptionExpiresAt extracts subscription expiration from OpenAI auth claims.
// Returns empty string if not present or invalid. Handles:
// - RFC3339 string: "2026-05-02T20:32:12+00:00"
// - Unix timestamp number (int64/float64)
// - Invalid/empty values are silently ignored (returns "")
func (c *IDTokenClaims) ExtractSubscriptionExpiresAt() string {
	if c == nil || c.OpenAIAuth == nil || c.OpenAIAuth.SubscriptionExpiresAtRaw == nil {
		return ""
	}

	switch v := c.OpenAIAuth.SubscriptionExpiresAtRaw.(type) {
	case string:
...
// ExtractSubscriptionExpiresFromIDToken extracts subscription expiration from an id_token JWT.
// Safe for invalid/expired tokens - never panics, returns empty string on any error.
func ExtractSubscriptionExpiresFromIDToken(idToken string) string {
	idToken = strings.TrimSpace(idToken)
	if idToken == "" {
		return ""
	}
	// Use DecodeIDToken which doesn't validate expiration
	claims, err := DecodeIDToken(idToken)
	if err != nil {
		return ""
	}
	return claims.ExtractSubscriptionExpiresAt()
}
